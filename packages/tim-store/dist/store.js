 ...
 */
const MACHINE_STAMPED_TAGS = new Set([commit_tree_js_1.COMMIT_TAG, '#commits']);
/**
...
class TimStore {
     ...
     */
    async purgeStaging(vacuum = false) {
     ...
     */
    async searchByTag(tag, topK = 10, project) {
     ...
     */
    async projectTagVocabulary(project) {
        const resolved = await this.resolveProjectLabel(project);
        if (resolved.status !== 'found')
            return [];
        const root = await this.read(resolved.label);
    ...
    `).all(root.id);
        ...
        for (const row of rows) {
            ...
            if (!Array.isArray(parsed))
                continue;
            for (const tag of parsed) {
                if (typeof tag !== 'string' || session_tree_js_1.BATCH_STRUCTURAL_TAGS.has(tag))
                    continue;
                if (MACHINE_STAMPED_TAGS.has(tag))
                    continue;
