...
function replaceIdInJsonString(json, oldId, newId) {
...
class CurateManager {
    ...
    renameEntry(oldId, newId) {
        ...
        const transaction = this.db.transaction(() => {
            ...
            if (isBatchSummary) {
                this.db.prepare('UPDATE entries SET tombstoned_at = ? WHERE id = ?').run(new Date().toISOString(), oldId);
            }
      ...
      `).run(newId, oldId);
            ...
            // Child entries
            const children = this.db.prepare('SELECT id FROM entries WHERE parent_id = ?').all(oldId);
            for (const child of children)
                affectedEntryIds.add(child.id);
            ...
            const stagingRows = this.db.prepare("SELECT rowid, payload FROM staging WHERE payload LIKE '%' || ? || '%'").all(oldId);
            ...
            this.db.prepare("UPDATE suppressed SET suppressed_by = replace(suppressed_by, ?, ?) WHERE suppressed_by LIKE '%' || ? || '%'").run(oldId, newId, oldId);
            // Metadata JSON best-effort scan — entries
            const metaRows = this.db.prepare("SELECT id, metadata FROM entries WHERE metadata LIKE '%' || ? || '%'").all(oldId);
            const updateMeta = this.db.prepare('UPDATE entries SET metadata = ? WHERE id = ?');
            for (const row of metaRows) {
                if (row.id === oldId || row.id === newId)
                    continue;
                updateMeta.run(replaceIdInJsonString(row.metadata, oldId, newId), row.id);
                affectedEntryIds.add(row.id);
            }
            // Edge metadata
            const edgeMetaRows = this.db.prepare("SELECT id, metadata FROM edges WHERE metadata LIKE '%' || ? || '%'").all(oldId);
    ...
    moveEntry(id, newParentId, order) {
        ...
        `).all(newParentId, id);
                for (const sibling of siblings) {
                    const sibMeta = JSON.parse(sibling.metadata);
    ...
    tagRename(oldTag, newTag, opts = {}) {
        ...
        `).all(opts.rootId, oldTag)
            : this.db.prepare("SELECT id, tags FROM entries WHERE tags LIKE '%' || ? || '%'").all(oldTag));
        const transaction = this.db.transaction(() => {
            let count = 0;
...
exports.CurateManager = CurateManager;
