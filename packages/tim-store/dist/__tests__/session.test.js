...
(0, vitest_1.describe)('SessionManager', () => {
    ...
    (0, vitest_1.describe)('recentActiveProjects', () => {
        ...
        (0, vitest_1.it)('drops projects whose project entry was deleted (soft or hard)', async () => {
            vitest_1.vi.useFakeTimers();
            ...
            for (const [i, label] of ['P0091', 'P0092', 'P0093'].entries()) {
                vitest_1.vi.setSystemTime(new Date(`2026-07-0${i + 1}T10:00:00Z`));
                await sessions.startProjectSession({
            ...
            const recents = await store.recentActiveProjects(5);
            (0, vitest_1.expect)(recents.map(r => r.label)).toEqual(['P0091']);
        });
    });
});
