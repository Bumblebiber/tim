...
function getSweepAttempts(metadata) {
    const v = metadata[SWEEP_ATTEMPTS_KEY];
    return typeof v === 'number' && v >= 0 ? v : 0;
}
function getSweepBatchesAtSpawn(metadata) {
    const v = metadata[SWEEP_BATCHES_AT_SPAWN_KEY];
    return typeof v === 'number' ? v : undefined;
}
async function patchSessionSweepMetadata(store, sessionId, patch) {
    const session = await store.read(sessionId);
    if (!session)
        throw new Error(`Session not found: ${sessionId}`);
    const next = { ...session.metadata };
    if (patch.sweepAttempts !== undefined) {
        if (patch.sweepAttempts <= 0)
            next[SWEEP_ATTEMPTS_KEY] = null;
        else
            next[SWEEP_ATTEMPTS_KEY] = patch.sweepAttempts;
    }
    if (patch.sweepBatchesAtSpawn !== undefined) {
        if (patch.sweepBatchesAtSpawn === null)
            next[SWEEP_BATCHES_AT_SPAWN_KEY] = null;
        else
            next[SWEEP_BATCHES_AT_SPAWN_KEY] = patch.sweepBatchesAtSpawn;
    }
    ...
    for (const session of sessions) {
        ...
        if (attempts >= maxAttempts) {
            ...
            if (!sweepSkipLogged.has(key)) {
                errorLogger.logError({
                    tool: 'idle_sweep',
                    error: `session exhausted ${maxAttempts} idle-sweep attempts without a new summary — skipped`,
                    sessionId,
                });
                sweepSkipLogged.add(key);
            }
