 ...
 */
async function recentExchanges(store, sessionId, maxChars) {
    if (maxChars <= 0)
        return [];
    ...
    const summaries = summaryNode
        ? await store.getChildByKind(summaryNode.id, tim_store_1.KIND_BATCH)
        : [];
    const seqFloor = summaries.reduce((max, s) => Math.max(max, Number(s.metadata.seq_to) || 0), 0);
    const batches = await store.getChildByKind(exNode.id, tim_store_1.KIND_EXCHANGE_BATCH);
    const users = [];
    for (const batch of batches) {
        users.push(...(await store.getChildrenBySeq(batch.id)).filter(u => u.metadata.role === 'user'));
    }
    const tail = users
    ...
    for (const user of [...tail].reverse()) {
        ...
        if (agent)
            lines.push(`  ↳ ${oneLine(entryText(agent), RECENT_EXCHANGE_SIDE_MAX_CHARS)}`);
...
async function previousSession(store, projectLabel, maxChars, rawMaxChars) {
    ...
    if (!body && summaryNode) {
        const batchSummaries = await store.getChildByKind(summaryNode.id, tim_store_1.KIND_BATCH).catch(() => []);
        body = batchSummaries
            .slice()
