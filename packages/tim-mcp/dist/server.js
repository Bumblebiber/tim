...
async function resolveMarkerSession(store, cwd) {
    const located = findConfiguredMarker(cwd);
    if (!located)
        return undefined;
    const session = await (0, tim_store_1.resolveCurrentSession)(store, located.marker.project, cwd);
    return session?.id;
}
async function resolveHarnessSessionId(store, options) {
...
const TimSearchSchema = zod_1.z.object({
    ...
    excerptChars: zod_1.z.number().int().min(0).optional().default(500)
        .describe('Maximum Unicode code points per result excerpt; values above 500 are clamped to 500, ' +
...
const TimCheckpointSchema = zod_1.z.object({
    sessionId: zod_1.z.string(),
    handoff_note: zod_1.z.string().optional()
...
const TimLoadProjectSchema = zod_1.z.object({
    ...
    sessionId: zod_1.z.string().optional().describe('Harness session id; when omitted, resolved from env/cache or the current session for the marker-bound project (stdio only). Binds TIM session project_ref on first load only.'),
...
async function createMcpServer(options = {}) {
    ...
    const callerProjectPath = (0, project_path_js_1.resolveCallerProjectPath)(isHttp);
    // ─── Tool Registration ──────────────────────────────
            ...
            switch (name) {
                case 'tim_read': {
                    ...
                    if (Array.isArray(id)) {
                        ...
                        bestEffortTelemetry('recordRead', () => s.recordRead(entries.map(e => e.id), usageSid));
                        return {
                    ...
                    if (project && id === undefined) {
                        ...
                        bestEffortTelemetry('recordRead', () => s.recordRead([entry.id], usageSid));
                        return {
                            content: [{ type: 'text', text: formatToolResponse({
                ...
                case 'tim_search': {
                    ...
                    const response = {
                        ...(0, search_response_js_1.buildBoundedSearchResponse)(results, excerptChars),
                        ...(clamped ? { clamped } : {}),
                    };
                    bestEffortTelemetry('recordRead', () => s.recordRead(response.results.map(e => e.id), usageSid));
                    return {
                ...
                case 'tim_link': {
                    ...
                    if (refIds.length > 0) {
                        bestEffortTelemetry('markReferenced', () => s.markReferenced(refIds, usageSid));
                    }
                    return {
                ...
                case 'tim_update': {
                    ...
                    if (patch.tags !== undefined) {
                        ...
                        bestEffortTelemetry('markReferenced', () => s.markReferenced([entry.id], usageSid));
                ...
                case 'tim_create_project': {
                    ...
                    for (let attempt = 0; attempt < 10; attempt++) {
                        try {
                            ...
                            if ((0, tim_store_1.isProjectLabelConflictError)(err) && attempt < 9) {
                                label = (0, tim_store_1.nextLabelAfterProjectLabelConflict)(s, label);
                                continue;
                            }
                            throw err;
                        }
