...
(0, vitest_1.describe)('MCP tool response JSON safety (BUG 2)', () => {
    let client;
    let dbPath;
    ...
    (0, vitest_1.it)('tim_stats returns parseable JSON (no # chars in numbers/booleans)', async () => {
        const resp = await client.callTool('tim_stats');
        (0, vitest_1.expect)(resp.error).toBeUndefined();
        const text = resp.result.content[0].text;
        // Must be valid JSON.
        const stats = JSON.parse(text);
        (0, vitest_1.expect)(typeof stats.totalEntries).toBe('number');
    });
    (0, vitest_1.it)('tim_write + tim_read round-trip: response is parseable JSON even with # in tags', async () => {
        ...
        (0, vitest_1.expect)(written.tags).toEqual(['#session-summary', '#batch-summary']);
        ...
        (0, vitest_1.expect)(read.entry.tags).toEqual(['#session-summary', '#batch-summary']);
    });
    (0, vitest_1.it)('tim_search returns parseable JSON even when entries have # in tags', async () => {
        ...
        if (results.length > 0) {
            ...
            (0, vitest_1.expect)(typeof results[0].tags).toBe('object'); // array
        }
    });
    (0, vitest_1.it)('tim_load_project response is plain text (not JSON) — tags are inlined, not parsed', async () => {
        ...
        (0, vitest_1.expect)(loadResp.error).toBeUndefined();
    ...
    (0, vitest_1.it)('every tool response is either valid JSON or a clean text error message', async () => {
        ...
        for (const t of tools) {
            ...
            if (resp.error) {
                (0, vitest_1.expect)(typeof resp.error.message).toBe('string');
                continue;
            }
            const text = resp.result.content[0].text;
            ...
            if (isError) {
                ...
                (0, vitest_1.expect)(looksLikeJson || text.startsWith('Error'), `tool ${t.name} returned isError:true but text is neither JSON nor "Error: ...": ${text.slice(0, 80)}`).toBe(true);
            }
            else {
                // Success — must be valid JSON.
                (0, vitest_1.expect)(() => JSON.parse(text), `tool ${t.name} returned non-JSON on success: ${text.slice(0, 80)}`).not.toThrow();
            }
        }
    });
});
//# sourceMappingURL=stats-output.test.js.map