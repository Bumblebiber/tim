...
function projectMetaLine(project, parsed) {
    ...
    if (parsed.packages != null)
        bits.push(`${parsed.packages} packages`);
    if (parsed.tests != null)
        bits.push(`${parsed.tests} tests`);
    return bits.join(' · ');
}
...
function parseSessionEntry(entry, sessionNode) {
    ...
    const isSentinel = entry.title === tim_store_1.SUMMARY_NODE_TITLE;
...
function buildChildMap(children) {
    const map = new Map();
...
function formatSectionLineSuffix(section, subkids, renderDepth) {
